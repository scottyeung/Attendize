...
<html lang="en">
    <head>
        <title>{{{$event->title}}} - Inverted Audio</title>
       ...
       {!!HTML::style(config('attendize.cdn_url_static_assets').'/assets/stylesheet/frontend.css')!!}
        ...
        <style>
            #order_form {background: #95929D !important;}
            #details, #share {background-color: {{ $event->section_color }} !important; }
            #details, #share, #order_form, #order_form .panel > div, .help-block, .for-event-page {color: {{ $event->text_color }} !important; }
            .panel-primary>.panel-heading {border-color: #ffffff;}
            #order_form .for-event-page > .row {
                padding: 20px;
            }
        </style>
